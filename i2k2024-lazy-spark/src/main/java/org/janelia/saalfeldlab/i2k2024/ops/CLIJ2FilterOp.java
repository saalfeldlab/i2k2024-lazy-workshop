...
import org.janelia.saalfeldlab.i2k2024.util.Util;
...
public class CLIJ2FilterOp<T extends RealType<T> & NativeType<T>, S extends RealType<S>> implements Consumer<RandomAccessibleInterval<T>> {
	...
	public CLIJ2FilterOp(
		...
		this.source = source;
		final var n = source.numDimensions();
		if (n == padding.length)
	...
	public void accept(final RandomAccessibleInterval<T> cell) {
		...
		final var rai2cl = new RandomAccessibleIntervalToClearCLBufferConverter();
		rai2cl.setCLIJ(clij2.getCLIJ());

		final var input = rai2cl.convert(Views.interval(source, Intervals.expand(cell, padding)));
		final var output = clij2.create(input);

		filter.accept(input, output);

		final var cl2rai = new ClearCLBufferToRandomAccessibleIntervalConverter();
		cl2rai.setCLIJ(clij2.getCLIJ());
		final var result = cl2rai.convert(output);
