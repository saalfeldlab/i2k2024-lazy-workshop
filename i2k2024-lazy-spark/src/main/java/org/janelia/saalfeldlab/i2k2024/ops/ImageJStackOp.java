...
public class ImageJStackOp<T extends RealType<T> & NativeType<T>> implements Consumer<RandomAccessibleInterval<T>> {
	...
	public void accept(final RandomAccessibleInterval<T> output) {
		...
		final T type = output.getType().createVariable();
		final int n = output.numDimensions();
		final var min = Intervals.minAsLongArray(output);
		final var max = Intervals.maxAsLongArray(output);
		...
		final var inputInterval = Views.interval(
				Converters.convert(
						input,
						(a, b) -> {
		...
		var inputSlice = inputInterval;
		var outputSlice = output;
		final var slicePosition = min.clone();
		...
		for (int d = 2; d < n;) {
			...
			final var fp = Util.materialize(inputSlice);
			fp.setMinAndMax(minIntensity, maxIntensity);
			final var imp = new ImagePlus("", fp);
